/**
 * Proves the doll actually makes noise, and the right kind of noise.
 *
 *   node test-sound.mjs
 *
 * You cannot listen to a headless browser, so this counts what the page asks
 * the speaker to do. Every sound here is built out of Web Audio nodes, so a
 * sound that plays creates nodes and a sound that does not, does not. The
 * counting is done by wrapping AudioContext before the page's own code runs.
 *
 * The bug it exists for: burning and being loved are STATES that last, but
 * they were played as one-shots on the button press. You lit the doll on fire,
 * heard a single puff, and then watched him burn in silence - which is exactly
 * what "the burning sound is missing" means. A test that only checked "does
 * pressing fire make a sound" would have passed the whole time.
 */
import { chromium } from 'playwright';
import { spawn } from 'child_process';
import path from 'path';
import { fileURLToPath } from 'url';

const ROOT = path.dirname(fileURLToPath(import.meta.url));
const PORT = 8895;
const errs = [];
const check = (label, got, want) => {
  const ok = got === want;
  if (!ok) errs.push(`${label}\n    got:  ${JSON.stringify(got)}\n    want: ${JSON.stringify(want)}`);
  console.log(`  ${ok ? 'ok  ' : 'FAIL'} ${label}`);
};

/* Serve public/, which is what Cloudflare serves. The standalone beatass.html
   at the repo root has no sfx folder beside it and falls back to synthesised
   sound on purpose - that path is covered by the second pass below. */
const server = spawn('python3', ['-m', 'http.server', String(PORT), '--bind', '127.0.0.1'],
  { cwd: path.join(ROOT, 'public'), stdio: 'ignore', detached: true });
const stop = () => { try { process.kill(-server.pid, 'SIGTERM'); } catch {} };
process.on('exit', stop);

const browser = await chromium.launch({ args: ['--autoplay-policy=no-user-gesture-required'] });
const page = await browser.newPage();

/* Count every sound the page starts, before any of its own code runs. */
const initScript = () => {
  window.__sfx = { starts: 0, loops: 0, running: 0, loopsRunning: 0, samples: 0 };
  const Real = window.AudioContext || window.webkitAudioContext;
  window.AudioContext = window.webkitAudioContext = function () {
    const ac = new Real();
    const bs = ac.createBufferSource.bind(ac);
    ac.createBufferSource = function () {
      const s = bs();
      const start = s.start.bind(s), stopFn = s.stop.bind(s);
      s.start = function (...a) {
        window.__sfx.starts++; window.__sfx.running++;
        if (s.loop) { window.__sfx.loops++; window.__sfx.loopsRunning++; }
        /* a recorded effect is a buffer we decoded; a synthesised one is a
           buffer we filled in by hand. The decoded ones are stereo. */
        if (s.buffer && s.buffer.numberOfChannels > 1) window.__sfx.samples++;
        return start(...a);
      };
      s.stop = function (...a) { window.__sfx.running--; if (s.loop) window.__sfx.loopsRunning--; return stopFn(...a); };
      return s;
    };
    const os = ac.createOscillator.bind(ac);
    ac.createOscillator = function () {
      const o = os();
      const start = o.start.bind(o);
      o.start = function (...a) { window.__sfx.starts++; return start(...a); };
      return o;
    };
    return ac;
  };
};
await page.addInitScript(initScript);

const count = () => page.evaluate(() => ({ ...window.__sfx }));

/* Wait for the little web server to actually be listening. Locally it is up
   before the browser finishes launching, so this looked unnecessary; on CI the
   machine is slower and the first navigation was refused. A test that depends
   on who wins a race is not a test. */
async function waitForServer() {
  for (let i = 0; i < 60; i++) {
    try {
      const r = await fetch(`http://127.0.0.1:${PORT}/index.html`, { method: 'HEAD' });
      if (r.ok) return;
    } catch {}
    await new Promise((r) => setTimeout(r, 500));
  }
  throw new Error(`the test web server never came up on port ${PORT}`);
}

try {
  await waitForServer();
  /* count the recordings as they arrive, so the test can wait for them
     instead of racing them */
  let sfxLoaded = 0;
  page.on('response', (r) => { if (/\/sfx\/.*\.mp3$/.test(r.url()) && r.ok()) sfxLoaded++; });
  await page.goto(`http://127.0.0.1:${PORT}/index.html`, { waitUntil: 'networkidle' });

  /* reach the doll the same way a person does */
  await page.fill('#i-name', 'Test');
  await page.fill('#i-email', 'test@example.com');
  await page.fill('#i-msg', 'a message long enough to be allowed through to the doll screen.');
  await page.locator('button:has-text("next")').first().click().catch(() => {});
  await page.waitForTimeout(800);
  const doll = await page.locator('#doll').boundingBox();
  if (!doll) throw new Error('never reached the doll');

  /* the recordings download on the first touch; give them a moment to land */
  for (let i = 0; i < 40 && sfxLoaded < 7; i++) await page.waitForTimeout(250);
  console.log(`\n${sfxLoaded} of 7 recordings loaded`);
  check('the recorded effects are actually downloaded by the site', sfxLoaded, 7);

  console.log('\nhitting him:');
  const before = await count();
  await page.mouse.click(doll.x + doll.width * 0.5, doll.y + doll.height * 0.45);
  await page.waitForTimeout(600);
  const afterPunch = await count();
  /* a punch is layered on purpose - one node is a beep, several is a hit */
  check('a punch makes a sound', afterPunch.starts - before.starts >= 1, true);
  check('the punch is the recorded one, not the fallback', afterPunch.samples > before.samples, true);

  await page.click('.tool[data-tool="pin"]');
  await page.waitForTimeout(300);
  const beforePin = await count();
  await page.mouse.click(doll.x + doll.width * 0.5, doll.y + doll.height * 0.5);
  await page.waitForTimeout(600);
  const afterPin = await count();
  check('a pin makes a sound', afterPin.starts - beforePin.starts >= 1, true);
  check('the pin is the recorded one', afterPin.samples > beforePin.samples, true);

  console.log('\nsetting him on fire:');
  await page.click('.tool[data-tool="fire"]');
  await page.waitForTimeout(1200);
  const litUp = await count();
  check('lighting the fire starts a looping sound', litUp.loops >= 1, true);

  /* THE test, and it has to pass whether the fire is a recording (one long
     looping file) or the synthesised fallback (a loop plus crackles), so it
     asks the only question that means the same in both: is a looping sound
     STILL playing two and a half seconds after he was lit? */
  const t1 = await count();
  await page.waitForTimeout(2500);
  const t2 = await count();
  console.log(`      (${t2.loopsRunning} loop still running, ${t2.starts - t1.starts} extra sounds in 2.5s)`);
  check('the fire is STILL burning 2.5s later, it is not one puff', t2.loopsRunning >= 1, true);

  console.log('\nputting it out:');
  await page.click('.tool[data-tool="fire"]');
  await page.waitForTimeout(1200);
  await page.waitForTimeout(1200);
  const t4 = await count();
  check('the fire stops when it is put out', t4.loopsRunning, 0);
  const q = await count();
  await page.waitForTimeout(1800);
  check('and stays quiet', (await count()).starts - q.starts, 0);

  console.log('\nloving him:');
  await page.click('.tool[data-tool="love"]');
  await page.waitForTimeout(700);
  const l1 = await count();
  check('love makes a sound', l1.starts - t4.starts >= 1, true);
  await page.waitForTimeout(3200);
  const l2 = await count();
  check('being loved keeps making a soft sound', l2.starts - l1.starts >= 1, true);
  await page.click('.tool[data-tool="love"]');
  await page.waitForTimeout(1200);
  const l3 = await count();
  await page.waitForTimeout(2000);
  check('it stops when you stop', (await count()).starts - l3.starts, 0);

  console.log('\nthe mute button:');
  const muteBtn = page.locator('#b-mute, [data-mute], button:has-text("sound")').first();
  if (await muteBtn.count()) {
    await muteBtn.click();
    await page.click('.tool[data-tool="fire"]');
    await page.waitForTimeout(2500);
    const m1 = await count();
    await page.waitForTimeout(2000);
    check('muted means silent, even while burning', (await count()).starts - m1.starts, 0);
  } else {
    console.log('  --   no mute control found to test');
  }
  /* ---- second pass: block the recordings, prove the fallback still works ----
     Without this the synthesised code would ship untested for ever, and it is
     exactly what runs for anyone offline or on a page opened off disk. */
  console.log('\nwith the recordings blocked (offline, or opened off disk):');
  const p2 = await browser.newPage();
  await p2.addInitScript(initScript);
  await p2.route('**/sfx/*.mp3', (r) => r.abort());
  await p2.goto(`http://127.0.0.1:${PORT}/index.html`, { waitUntil: 'networkidle' });
  await p2.fill('#i-name', 'Test');
  await p2.fill('#i-email', 'test@example.com');
  await p2.fill('#i-msg', 'a message long enough to be allowed through to the doll screen.');
  await p2.locator('button:has-text("next")').first().click().catch(() => {});
  await p2.waitForTimeout(800);
  const d2 = await p2.locator('#doll').boundingBox();
  const c2 = () => p2.evaluate(() => ({ ...window.__sfx }));
  const b2 = await c2();
  await p2.mouse.click(d2.x + d2.width * 0.5, d2.y + d2.height * 0.45);
  await p2.waitForTimeout(600);
  const a2 = await c2();
  check('a punch still lands with no recordings', a2.starts - b2.starts >= 4, true);
  check('and it is the synthesised one', a2.samples, b2.samples);
  await p2.click('.tool[data-tool="fire"]');
  await p2.waitForTimeout(1200);
  const f1 = await c2();
  await p2.waitForTimeout(2500);
  const f2 = await c2();
  check('the synthesised fire keeps crackling too', f2.starts - f1.starts >= 5, true);
  check('and its loop is still running', f2.loopsRunning >= 1, true);
  await p2.close();
} catch (err) {
  errs.push('the sound test could not finish: ' + err.message.split('\n')[0]);
  await page.screenshot({ path: ROOT + '/shots/sound-failure.png' }).catch(() => {});
} finally {
  await browser.close();
  stop();
}

if (errs.length) console.error('\nSOUND ERRORS:\n  ' + errs.join('\n  ') + '\n');
else console.log('\nsound: all checks pass');
process.exit(errs.length ? 1 : 0);
