...
// Music arc: clip A (playful-dark) opens, clip B (dread build) takes over at
// the confession (S5, 21.7s) and runs to the end. SFX on the v3 beat times.
const MUSIC_A_END = Math.round(25.3 * 30);
const MUSIC_B_FROM = Math.round(21.7 * 30);
...
const SFX: Array<{file: string; from: number; volume: number}> = [
  ...
  {file: 'sfx/sub-pulse-soft.mp3', from: Math.round(25.3 * 30), volume: 0.32},
  {file: 'sfx/sting-soft.mp3', from: Math.round(46.5 * 30), volume: 0.32},
];
  ...
  <AbsoluteFill style={{backgroundColor: system.bg}}>
    ...
    <Sequence from={MUSIC_B_FROM}>
      <Audio
        ...
        volume={(f) => interpolate(f, [0, 120, 784, 844], [0, 0.14, 0.14, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})}
      />
    </Sequence>
    {SFX.map((s) => (
