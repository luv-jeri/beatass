...
export const schema = {
  src: {type: 'text', label: 'Clip file (public/)', default: 'clips/current/scene-7.mp4'},
  startFromSeconds: {type: 'number', label: 'Clip start offset (s)', default: 3.5, min: 0, max: 8, step: 0.1},
  clipSeconds: {type: 'number', label: 'Source clip length (s)', default: 4.5, min: 1, max: 30, step: 0.5},
  sfxVolume: {type: 'number', label: 'Clip SFX volume', default: 0.22, min: 0, max: 1, step: 0.05},
} as const;

export const durationInFrames = 87;
